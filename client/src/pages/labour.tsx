...
export default function LabourPage() {
  ...
  const [editingLabourGroup, setEditingLabourGroup] = useState<LabourGroup | null>(null);
  ...
  const { data: labours, isLoading } = useQuery<Labour[]>({ // Renamed 'labour' to 'labours' for clarity
    queryKey: ['/api/labour'],
  });
  ...
  const handleLabourEdit = (labourData: Labour) => { // Renamed for clarity
    setEditingLabour(labourData);
    setIsDialogOpen(true);
  };

  const handleDeleteLabour = (id: number) => { // Specific handler for Labour
    if (confirm('Are you sure you want to delete this labour record?')) {
      deleteLabourMutation.mutate(id);
    }
  };

  const handleDeleteLabourGroup = (id: number) => { // Specific handler for LabourGroup
  ...
  const getStatusColor = (status: string) => {
    return status === 'active' ? 'bg-green-100 text-green-800 hover:text-white' : 'bg-red-100 text-red-800 hover:text-white';
  };
      ...
      <div className="flex-1 ml-64">
        ...
        <main className="p-6">
          <div className="mb-6 flex justify-between items-center">
            <div>
              <h3 className="text-lg font-semibold text-gray-800">Labour Management</h3> {/* Changed title for clarity */}
              <p className="text-sm text-gray-600">Manage your individual labour and labour teams</p>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200">
            <div className="overflow-x-auto">
              <Tabs
                ...
                className="space-y-6"
              >
                <TabsList className="grid w-full grid-cols-2"> {/* Adjusted grid-cols to 2 */}
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                          ...
                          {labourGroups?.map((labourGroup: LabourGroup, idx: number) => (
                            <tr key={labourGroup.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm font-medium text-gray-900">{labourGroup.groupName}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
                                <div className="text-sm font-medium text-gray-900">{labourGroup.description}</div>
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                <Button
                                ...
                                <Button
                                  onClick={() => handleDeleteLabourGroup(labourGroup.id)} // Call specific delete handler
                                  variant="destructive"
                                  size="sm"
